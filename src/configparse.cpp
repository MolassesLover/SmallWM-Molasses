...
#undef SYSLOG_MACRO_CHECK
            ...
            if (value.size() > 0) self->log_file = value;
        } else if (name == std::string("hotkey-mode")) {
            if (value == std::string("focus")) self->hotkey = HK_FOCUS;
            else if (value == std::string("mouse")) self->hotkey = HK_MOUSE;
            else self->hotkey = HK_MOUSE;
        } else if (name == std::string("shell")) {
            if (value.size() > 0) self->shell = value;
            ...
            if (value.size() > 0) self->dump_file = value;
        }
        ...
        do{
            ...
            if (!strcmp(stripped, "stick")) {
                ...
                if (!strncmp(stripped + 5, "NW", 2)) action.pack_corner = PACK_NORTHWEST;
                else if (!strncmp(stripped + 5, "NE", 2)) action.pack_corner = PACK_NORTHEAST;
                else if (!strncmp(stripped + 5, "SW", 2)) action.pack_corner = PACK_SOUTHWEST;
                else if (!strncmp(stripped + 5, "SE", 2)) action.pack_corner = PACK_SOUTHEAST;
                else is_valid = false;
 ...
 finish_actions:
    ...
    // more specifically KeyboardConfig.
        ...
        if (uses_secondary_action) key_value.erase(0, 1);
        ...
        if (kb_config.binding_to_action[binding] != INVALID_ACTION) return 0;

        // If an old binding exists for this action, then remove it
        KeyBinding old_binding = kb_config.action_to_binding[action];

        if (old_binding.first != NoSymbol) kb_config.binding_to_action.erase(old_binding);
